...
#################### Annual climate data from geo coordinates (WGS 84) ##########################"
#################################################################################################"
...
get.climate<-function(xy,clim.var,period=1901:2014,fun=NULL,radius=NULL,dir.in,download=T,dir.out,n.core) {
  ...
  if(is.null(clim.var) | clim.var=="all") clim.var=eumedclim.vars else
...
### Run 
ex=get.climate(xy=coord,clim.var="all",dir.in=dir.in,download=T,dir.out=dir.out,n.core=3)
...
# Warning message:
#  In mclapply(as.list(clim.var), function(k) { :
#      scheduled cores 3, 1 encountered errors in user code, all values of the jobs will be affected
    
# plot time series
for(k in para) {
  
  ylim=range(ex[[k]][,-c(1:2)])
  plot(c(1901,2014),ylim,t='n',xlab="",ylab=k)
  lines(1901:2014,ex[[k]]['bordeaux',-c(1:2)],col=4)
  lines(1901:2014,ex[[k]]['madrid',-c(1:2)],col=2)
  if(k==para[1]) legend('topleft',bty='n',lty=1,col=c(4,2),legend=c('Bordeaux','Madrid'))
}
