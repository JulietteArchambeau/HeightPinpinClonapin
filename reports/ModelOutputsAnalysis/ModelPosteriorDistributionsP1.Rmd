...
```{r LoadDATA}
data <- readRDS(file="../../data/datatrain_pheno-clim-soil_20012020_Sample2_75percent.RDS")
```
...
df %>% mutate(Parameter = recode_factor(rownames(df),'b_age.sc'="$\\beta_{age}$",
                              ...
                              'sd_clon__Intercept'='$\\sigma_{G_{P}}$',
                              'sd_site__Intercept'='$\\sigma_{S}$',
                              'sd_site:block__Intercept'='$\\sigma_{B_{S}}$')) %>% 
  dplyr::select(Parameter,Mean,SD,InfCI,SupCI) %>%
  remove_rownames() %>% 
...
df <- df %>% mutate(Parameter = recode_factor(rownames(df),'sigma'='$\\sigma^{2}$',
                              'sd_prov__Intercept'="$\\sigma^{2}_{P}$",
                              'sd_clon__Intercept'='$\\sigma^{2}_{G_{P}}$',
...
p_sigmaAj <- ggplot()+
  ...
  labs(y  = "", 
       x  = TeX("Variances $\\sigma_{A_{j}}^{2}$ of the partial breeding values $a_{gj}$ from gene pool $j$")) +
...
```{r M5PartialBreedingValues, warning=F,message=F}
...
ggsave(pp,file="../../figs/manuscript/AddGenVarM5.png",height=12,width=12)
pp 
```
...
```{r savePostDHPC}
ggsave(gg, file="../../figs/manuscript/PostDHPC.png",width = 16,height=16)   
```
