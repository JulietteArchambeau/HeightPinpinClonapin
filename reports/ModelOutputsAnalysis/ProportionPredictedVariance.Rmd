...
```{r Visualization5,fig.height=12,fig.width=10}
# Figure asked by the Co-editor of the American Naturalist
p <- df %>% 
  bind_rows() %>% 
  mutate(site=replace(site, site=="caceres", "Cáceres")) %>% 
  pivot_longer(cols=contains("MOD"),names_to = "model",values_to="var") %>% 
...
```{r VisualizationExplicativePart2,fig.height=10,fig.width=12}
# Figure asked by the Co-editor of the American Naturalist
p <- df %>% 
  bind_rows() %>% 
  mutate(site=replace(site, site=="caceres", "Cáceres")) %>% 
