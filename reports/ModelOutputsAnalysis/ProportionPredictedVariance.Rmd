...
In this document, we aim to calculate $\mathcal{R}^{2}_{ms}|age$ the proportion of the out-of-sample variance (variance of height growth in the test dataset, here 6 left provenances) that is predicted each model $m$ in each common garden $s$, conditional on the age effect.
...
Then, we calculate $P_s$ of model $m$ such as:

$$ \mathcal{R}^{2}_{ms}|age = \frac{Vpred_{ms} - Vage_{1s}}{Vy_{s} -Vage_{1s}}$$
where $Vy_s$ is the out-of-sample variance in height growth in site $m$ (in the test dataset, i.e. the 6 left provenances) and $Vpred_{ms}$ is the out-of-sample variance predicted by model $m$ in site $s$. 
...
```{r Vizualization1, fig.height=8,fig.width=10}
...
```{r Vizualization2, fig.height=12,fig.width=6}
df[[2]] %>% 
  dplyr::select(-MOD14) %>% 
  pivot_longer(cols=contains("MOD"),names_to = "model",values_to="var") %>% 
  mutate(model = paste0("M",str_sub(.$model,4,-1)),
...
```{r Vizualization3, fig.height=9,fig.width=17}
...
```{r Visualization4,fig.height=15,fig.width=20}
mypalette <- c( "#F46D43","#A6DBA0" ,"#5AAE61","#E0F3F8" , "#ABD9E9", "#74ADD1", "#4575B4")
